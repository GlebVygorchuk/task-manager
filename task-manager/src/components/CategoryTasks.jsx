...
export default function CategoryTasks({ title, onReturn, date, categoryId}) {
    ...
    useEffect(() => {
        if (userID && date) {
            const getCategoryTasks = onSnapshot(
                collection(database, 'users', userID, 'allTasks', date, 'categories', categoryId, 'category-tasks'),
                (querySnapshot) => {
    ...
    async function getCategoryColor() {
        setUpdating(true)
        const category = await getDoc(doc(database, 'users', userID, 'allTasks', date, 'categories', categoryId))
    ...
    return (
        ...
        {updating ? 
            ...
            style={{display: 'flex', justifyContent: 'center'}}>
                ...
                <svg 
                style={{position: 'initial', marginTop: '15px'}} 
                className="sandclock" 
        ...
        <div className="category-tasks">
            ...
            <div className="category-tasks__header">
                ...
                <h1 className="category-tasks__title"><span style={{color: currentColor === undefined ? 'black' : currentColor}} className="highlight">{title}</span></h1>
    ...
    <div className="category-tasks__return-btn__wrapper">
        ...
        <button 
        onClick={onReturn} 
        className="category-tasks__return-btn">
            &lt; назад
        </button>
