...
export default function TaskItem({ content, className, status, onStart, onFinish, onDelete, onSelect, operated, itemId, date }) {
    ...
    <div id={itemId} className="task-wrapper">
        ...
        <li className={className}>
            {isRedacting ? <input onBlur={() => handleEdit(itemId)} onInput={(e) => setInputValue(e.target.value)} value={inputValue} className="edit-task" /> : <><span className={status === 'complete' ? 'cross' : ''}>{taskContent}</span><br /><span className="task-status">{taskStatus}</span></>}
        </li>
        <svg onClick={onSelect} xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="#007bff" className="three-dots" viewBox="0 0 16 16">
            <path d="M9.5 13a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0m0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0m0-5a1.5 1.5 0 1 1-3 0 1.5 1.5 0 0 1 3 0"/>
        </svg>
        <div className={optionsState}>
            <svg onClick={onFinish} className="option" xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" viewBox="0 0 16 16">
                <path d="M10.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425z"/>
            </svg>
            <button onClick={onStart} style={{fontSize: '20px', fontWeight: '1000', userSelect: 'none'}} className="option">GO</button>
            ...
            <svg onClick={onDelete} className="option" xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" viewBox="0 0 16 16">
